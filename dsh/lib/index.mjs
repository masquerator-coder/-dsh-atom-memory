...
*/function defaultSpawn(pythonBin,cwd){const bin=pythonBin&&pythonBin.length>0?pythonBin:"python";return spawn(bin,["-m","atom_memory.rpc"],{stdio:["pipe","pipe","pipe"],cwd,env:{...process.env,PYTHONIOENCODING:"utf-8",PYTHONUNBUFFERED:"1"}});}/**
* A lightweight NDJSON request/response client for one bridge protocol.
*/var PythonBridge=class{deps;spawnProcess;onEvent;onLog;proc;incoming;outgoing;pending=/* @__PURE__ */new Map();nextId=1;disposed=false;constructor(deps){this.deps={timeoutMs:deps.timeoutMs??3e4,...deps};this.spawnProcess=deps.spawnProcess;this.onEvent=deps.onEvent;this.onLog=deps.onLog;}/** Whether a child process is currently alive. */get alive(){return this.proc!==void 0;}/**
...
*/const RAW_KNOWLEDGE_MIN_CHARS=120;/** Predicate stamped on raw-fallback knowledge facts. */const RAW_KNOWLEDGE_PREDICATE="知识";/** Longest title kept from the first line of a raw-fallback body. */const RAW_KNOWLEDGE_TITLE_CHARS=60;/**
...
*/function rawKnowledgeCandidate(text){const body=text.trim();const firstLine=body.split(/\r?\n/).map(l=>l.trim()).find(l=>l.length>0)??body;const title=firstLine.length<=RAW_KNOWLEDGE_TITLE_CHARS?firstLine:`${firstLine.slice(0,RAW_KNOWLEDGE_TITLE_CHARS)}…`;return{subject:"用户",predicate:RAW_KNOWLEDGE_PREDICATE,object:title,type:"sop",content:body};}/**
...
*/function sessionIdOf(exec,fallback){const sessionId=exec.agent?.session?.id;return sessionId!==void 0?sessionId:fallback;}/** Thrown when the memory master switch is off. */function disabledError(){return/* @__PURE__ */new Error("memory is disabled");}/** Register all memory tools and return their disposers. */function registerMemoryTools(deps){const{ctx,bridge}=deps;const disposers=[];const scope=deps.fallbackScope;const call=(method,params)=>bridge.call(method,params);disposers.push(ctx.tools.register(defineTool({name:"memory_add",description:"显式记住一条用户偏好、事实、事件、流程图或经验教训。传入原始内容，系统会自行抽取为原子事实。",parameters:{content:{type:"string",required:true,description:"要记住的原始内容"},user:{type:"string",description:"可选：归属用户 id（默认当前会话）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){const v=value;return[{type:"text",text:`已入队记忆 ${v.status??""} (${v.candidate_id??""})`}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);const sid=sessionIdOf(exec,scope);const raw=args.content;if(deps.extract!==void 0)try{const candidates=await deps.extract(raw);if(candidates.length>0)return{candidate_id:(await call("persist_candidates",{user_id:uid,session_id:sid,turn_id:0,candidates})).candidate_id??"",status:"queued"};}catch{}const body=raw.trim();if(body.length>=RAW_KNOWLEDGE_MIN_CHARS)return{candidate_id:(await call("persist_candidates",{user_id:uid,session_id:sid,turn_id:0,candidates:[rawKnowledgeCandidate(body)]})).candidate_id??"",status:"queued",fallback:"raw"};return await call("add",{user_id:uid,session_id:sid,text:raw,turn_id:0});}})));disposers.push(ctx.tools.register(defineTool({name:"memory_recall",description:"检索与查询相关的持久记忆原子事实。",parameters:{query:{type:"string",required:true,description:"要检索的记忆查询"},user:{type:"string",description:"可选：归属用户 id（默认当前会话）"},topK:{type:"integer",description:"返回条数上限（默认按配置）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){const v=value;const facts=v.facts??[];const summaries=(v.summaries??[]).map(s=>(s.text??"").trim()).filter(Boolean);const blocks=[];if(summaries.length>0)blocks.push(`【摘要】${summaries.join("；")}`);if(facts.length===0)blocks.push("（无相关记忆）");else blocks.push(facts.map(f=>{const head=[f.fact_id?`[${f.fact_id}]`:"",`${f.subject??""}${f.predicate??""}: ${f.object??""}`,f.type?`*(${f.type})*`:""].filter(Boolean).join(" ");const body=(f.content??"").trim();return body?`- ${head}\n    > ${body}`:`- ${head}`;}).join("\n"));return[{type:"text",text:blocks.join("\n")}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);const r=await call("recall",{user_id:uid,query:args.query,token_budget:4e3,top_k:args.topK??deps.maxRecalledFacts});return{facts:r.facts??[],summaries:r.summaries??[],token_count:r.token_count??0};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_summary",description:"查看当前用户记忆的聚合摘要（稳定属性、偏好、工作流程、近期事件、经验教训）。适合先看摘要，再按需用 memory_recall 查明细。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前会话）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:value.text??""}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);return{text:await call("summary",{user_id:uid})};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_forget",description:"软删除（retract）一条记忆。",parameters:{factId:{type:"string",description:"记忆 fact id（二选一）"},user:{type:"string",description:"可选：归属用户 id（默认当前会话）"}},output:{schema:{type:"object",additionalProperties:true},render(){return[{type:"text",text:"已处理该记忆"}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();if(!args.factId)throw new Error("memory_forget requires factId");return await call("forget",{user_id:args.user??userIdOf(exec,scope),fact_id:args.factId});}})));disposers.push(ctx.tools.register(defineTool({name:"memory_memory_md",description:"渲染当前用户的 memory.md（原子事实清单，含 fact_id）。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前会话）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:value.text??""}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);return{text:await call("memory_md",{user_id:uid,max_tokens:deps.memoryMdTokens})};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_user_md",description:"渲染当前用户的画像卡片 markdown。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前会话）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:value.text??""}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);return{text:await call("user_md",{user_id:uid})};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_stats",description:"返回当前用户的记忆统计计数。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前会话）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:JSON.stringify(value)}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();return await call("stats",{user_id:args.user??userIdOf(exec,scope)});}})));return disposers;}//#endregion
...
*/function registerCapture(deps,opts){const disposers=[];const{ctx,capture}=deps;const maxRecent=deps.maxRecent??20;const recent=/* @__PURE__ */new Map();const push=(sessionId,entry)=>{const list=recent.get(sessionId)??[];list.push(entry);while(list.length>maxRecent)list.shift();recent.set(sessionId,list);};/** Re-scan recent messages for those not yet captured. */const sweep=async sessionId=>{const list=recent.get(sessionId);if(!list)return;for(const entry of list){if(entry.captured)continue;entry.captured=true;await capture(entry.text,sessionId).catch(()=>{});}};if(opts.captureEnabled)disposers.push(ctx.on("session/event",(session,event)=>{if(event.type!=="user/message")return;if(!isDirectUserMessage(event))return;const text=userMessageText(event);if(text.trim().length===0)return;const entry={seq:event.seq??0,text,captured:false};entry.captured=true;push(session.id,entry);capture(text,session.id).catch(()=>{});}));if(opts.preCompressionCapture)disposers.push(ctx.on("llm/stream",async function*(options,next){if(options.purpose==="compaction"&&options.sessionId)try{await sweep(String(options.sessionId));}catch{}yield*await next();}));if(opts.nudgeEnabled){const timer=setInterval(()=>{for(const sessionId of recent.keys())sweep(sessionId).catch(()=>{});},Math.max(opts.nudgeIntervalMs,1e3));disposers.push(()=>clearInterval(timer));}return disposers;}//#endregion
...
*/function buildLlmExtractor(ctx,opts={}){const llm=ctx.get("llm");if(llm===void 0)return void 0;const def=ctx.get("agentDefaultModel");const modelOverride=opts.modelOverride?.();let provider=modelOverride?.provider?.trim()??"";let model=modelOverride?.model?.trim()??"";if(!provider&&def!==void 0)try{const selection=def.currentSelection();if(selection!==void 0){provider=selection.provider;model=selection.model;}}catch{}if(!provider||!model)return void 0;const enabled=opts.enabled;return async text=>{if(enabled?.()===false)return[];const messages=[createUserMessage({content:[{type:"text",text}],source:{kind:"plugin",plugin:"dsh-atom-memory"}})];const options={provider,model,messages,system:EXTRACTION_SYSTEM,maxTokens:opts.maxTokens??2048,purpose:"session-title"};const assembler=new BlockAssembler();for await(const chunk of llm.stream(options))assembler.push(chunk);const finished=assembler.finish;if(finished.kind!=="stop"){ctx.logger(`[atom-memory] extraction not persisted (finish=${finished.kind}); consider raising extractionMaxTokens (now ${opts.maxTokens??2048})`);return[];}const raw=assembler.blocks().filter(b=>b.type==="text").map(b=>b.text??"").join("").trim();if(!raw)return[];return parseCandidates(raw.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/,""));};}/**
...
* Python side.
*/var AtomMemoryController=class AtomMemoryController extends TypertRemoteService{static{[_initProto]=_applyDecs(this,[],[[Remote,2,"listFacts"],[Remote,2,"editFact"],[Remote,2,"listProfile"],[Remote,2,"upsertProfile"],[Remote,2,"deleteProfile"],[Remote,2,"backup"],[Remote,2,"restore"],[Remote,2,"getRuntime"]],0,void 0,TypertRemoteService).e;}bridge=void _initProto(this);runtime;constructor(ctx,bridge,runtime){super(ctx,"atomMemoryController",{namespace:"atom-memory"});this.bridge=bridge;this.runtime=runtime;}/** Whether the bridge is alive and the plugin master switch is on. */assertReady(){if(!this.runtime.isEnabled())throw new Error("memory is disabled");if(!this.bridge.alive)throw new Error("memory bridge is not running");}/** Paginate the user's active facts. */async listFacts(args){this.assertReady();return this.bridge.call("list_facts",{user_id:args.user,offset:args.offset??0,limit:args.limit??50,include_retracted:args.includeRetracted??false});}/** Directly edit one active fact's SPO / type / content. */async editFact(args){this.assertReady();if(!args.fact_id)throw new Error("editFact requires fact_id");return this.bridge.call("edit_fact",{user_id:args.user,fact_id:args.fact_id,subject:args.subject,predicate:args.predicate,object:args.object,content:args.content,type:args.type});}/** List the user's profile rows. */async listProfile(args){this.assertReady();return this.bridge.call("list_profile",{user_id:args.user});}/** Add or update one profile row. */async upsertProfile(args){this.assertReady();if(!args.section||!args.key)throw new Error("upsertProfile requires section and key");return this.bridge.call("upsert_profile",{user_id:args.user,section:args.section,key:args.key,value:args.value});}/** Delete one profile row. */async deleteProfile(args){this.assertReady();return this.bridge.call("delete_profile",{user_id:args.user,section:args.section,key:args.key});}/** Export the user's memory as a JSON snapshot (for download). */async backup(args){this.assertReady();return this.bridge.call("backup",{user_id:args.user});}/** Import a JSON snapshot, replacing the user's memory. */async restore(args){this.assertReady();if(!args.payload||typeof args.payload!=="object")throw new Error("restore requires a backup payload");return this.bridge.call("restore",{user_id:args.user,payload:args.payload});}/** Read the current live runtime (enabled / capture / model override). */async getRuntime(){return this.runtime.get();}};//#endregion
//#region src/index.ts
...
*/const inject=["tools","systemPrompt"];/** Fallback user/session scope for a single-user local harness. */const FALLBACK_SCOPE="global";/** Start params sent to the Python bridge (worker/embedding config). */function buildStartParams(config){return{db_path:config.dbPath??"~/.dsh/atom-memory/memory.db",worker_poll_interval_sec:.5,summary_rebuild_debounce_sec:5,max_retries:3};}/**
* Seed the live runtime from the composition config, applying defaults.
* @param config - the validated composition entry.
*/function seedRuntime(config){return createRuntime({enabled:config.enabled!==false,captureEnabled:config.captureEnabled!==false,llmExtractionEnabled:config.llmExtractionEnabled!==false,contextInjectionEnabled:config.contextInjectionEnabled!==false,extractionModel:config.extractionModel});}function apply(ctx,config){const runtime=new Runtime(seedRuntime(config));const bridge=new PythonBridge({spawnProcess:()=>defaultSpawn(config.pythonBin),timeoutMs:config.rpcTimeoutMs,onEvent:evt=>{ctx.logger(`[atom-memory] ${evt.evt} ${evt.candidate_id??""}`.trim());},onLog:msg=>ctx.logger(`[atom-memory] ${msg}`)});ctx.effect(()=>()=>{bridge.dispose();});const started={value:false,error:void 0,attempt:0};const tryStart=()=>{if(started.value)return;if(started.attempt>3){ctx.logger("[atom-memory] python bridge failed to start; memory offline");return;}started.attempt+=1;bridge.start(buildStartParams(config),void 0).then(()=>{started.value=true;started.error=void 0;ctx.logger(`[atom-memory] bridge ready (${(config.dbPath??"").trim()||"db"})`);}).catch(err=>{started.error=err;setTimeout(tryStart,1e3);});};if(config.autostart!==false)tryStart();const extract=runtime.get().llmExtractionEnabled===false?void 0:buildLlmExtractor(ctx,{maxTokens:config.extractionMaxTokens??2048,modelOverride:()=>runtime.get().extractionModel,enabled:()=>runtime.isEnabled()});try{new AtomMemoryController(ctx,bridge,runtime);}catch(err){ctx.logger(`[atom-memory] remote controller unavailable (${err?.message??err})`);}const capture=async(text,sessionId)=>{if(!runtime.isEnabled())return;if(started.value&&extract!==void 0)try{const candidates=await extract(text);if(candidates.length>0){await bridge.call("persist_candidates",{user_id:FALLBACK_SCOPE,session_id:sessionId,turn_id:0,candidates});return;}}catch{}if(started.value)await bridge.call("add",{user_id:FALLBACK_SCOPE,session_id:sessionId,text,turn_id:0});};const disposers=registerMemoryTools({ctx,bridge,fallbackScope:FALLBACK_SCOPE,maxRecalledFacts:config.maxRecalledFacts??10,memoryMdTokens:config.memoryMdTokens??1500,extract,isEnabled:()=>runtime.isEnabled()});for(const d of disposers)ctx.effect(()=>d);registerCapture({ctx,capture,maxRecent:20},{captureEnabled:runtime.get().captureEnabled,preCompressionCapture:config.preCompressionCapture!==false,nudgeEnabled:config.nudgeEnabled!==false,nudgeIntervalMs:(config.nudgeIntervalMinutes??30)*6e4}).forEach(d=>ctx.effect(()=>d));registerMemoryContext({ctx,bridge,userScope:FALLBACK_SCOPE,maxTokens:config.memoryMdTokens??1500,snapshotEnabled:runtime.get().contextInjectionEnabled,isEnabled:()=>runtime.isEnabled()});const settings=ctx.get("settings");if(settings?.installSection!==void 0){let source=()=>seedRuntime(config);settings.installSection(ctx,SETTINGS_NAMESPACE,LiveSettingsSchema,source(),{setSource:current=>{source=current;},onChange:()=>{runtime.set(source());}});ctx.logger(`[dsh-atom-memory] settings section "${SETTINGS_NAMESPACE}" registered`);}ctx.logger("[dsh-atom-memory] loaded");}/**
...
*/const LiveSettingsSchema=z.object({enabled:z.boolean().default(true),captureEnabled:z.boolean().default(true),llmExtractionEnabled:z.boolean().default(true),contextInjectionEnabled:z.boolean().default(true),extractionModel:z.object({provider:z.string().default(""),model:z.string().default("")}).default({provider:"",model:""})});//#endregion
