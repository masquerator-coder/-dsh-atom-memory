...
*   omitted.
*/async function extractViaEndpoint(deps){const{baseURL,model,apiKey,system,userText,maxTokens,log}=deps;const fetchImpl=deps.fetchImpl??globalThis.fetch;if(typeof fetchImpl!=="function")throw new Error("custom extraction endpoint requires a fetch implementation");const url=`${baseURL.replace(/\/+$/u,"")}/chat/completions`;log(`[atom-memory] extraction via custom endpoint ${baseURL} model=${model}`);const response=await fetchImpl(url,{method:"POST",headers:{"Content-Type":"application/json",Accept:"text/event-stream",...(apiKey?{Authorization:`Bearer ${apiKey}`}:{})},body:JSON.stringify({model,messages:[{role:"system",content:system},{role:"user",content:userText}],stream:true,max_tokens:maxTokens})});if(!response.ok)throw new Error(`custom endpoint ${baseURL} returned HTTP ${response.status??"error"}`);return collectSseText(response.body);}/**
